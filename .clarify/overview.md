...
1. **Aquarium_魚缸狀態的所有可能值為何** (High)
   - 原因：魚缸是系統核心實體，狀態定義影響所有相關功能

2. **Aquarium_魚缸尺寸的數值限制為何** (High)
   - 原因：尺寸驗證是魚缸建立的基礎邏輯

3. **通用_金額欄位是否允許負值或零值** (High)
   - 原因：影響多個實體的資料完整性

### 第二階段：核心功能規則
**處理主要業務流程的規則完整性**

4. **魚缸管理_刪除魚缸時是否需要檢查相關聯的資料** (High)
   - 原因：級聯刪除邏輯影響資料完整性策略

5. **魚隻管理_新增魚隻時是否需要檢查魚缸狀態** (High)
   - 原因：確保主要業務流程的前置條件清晰

6. **Equipment_設備狀態的所有可能值為何** (High)
   - 原因：設備狀態管理是重要的子功能

### 第三階段：安全性與業務邏輯
**處理安全相關的核心規則**

7. **餵食記錄管理_餵食記錄新增時是否需要檢查飼料狀態** (High)
   - 原因：食品安全相關，影響魚隻健康

8. **下藥記錄管理_下藥記錄新增時是否需要檢查藥品到期日** (High)
   - 原因：藥品安全相關，影響魚隻健康

9. **Fish_魚隻狀態的所有可能值為何** (High)
   - 原因：魚隻生命狀態管理的基礎
